...
export default function ApplicationsPage() {
  ...
  const filtered = useMemo(
    () => applications.filter((application) => statusFilter === "all" || application.status === statusFilter).sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)),
  ...
  return <div className="mx-auto max-w-[1080px] px-5 py-8 sm:px-8 lg:py-10">
    <header className="min-w-0">
      ...
      <p className="mt-3 max-w-2xl text-sm leading-6 text-[#687184]">Taslak ve gönderilmiş başvuruların yalnızca bu tarayıcıda saklanır. Gerçek proje sahiplerine bildirim gönderilmez.</p>
    </header>
          ...
          return <article key={application.id} className="flex min-w-0 max-w-full flex-col rounded-2xl border border-[#e1e2e8] bg-white p-5 shadow-[0_5px_16px_rgba(37,44,72,0.03)]">
            <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
              <div className="min-w-0">
                <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#757e90] [overflow-wrap:anywhere]">{project?.category ?? "Proje"}</p>
                <h2 className="mt-1.5 text-xl font-semibold tracking-[-0.035em] text-[#26334f] [overflow-wrap:anywhere]">{project?.name ?? "Bilinmeyen proje"}</h2>
              </div>
            ...
            <dl className="mt-5 grid min-w-0 grid-cols-1 gap-3 text-xs sm:grid-cols-2">
              ...
              <div className="min-w-0">
                ...
                <dd className="mt-1 font-semibold text-[#4f596c] [overflow-wrap:anywhere]">{application.weeklyAvailability}</dd>
              </div>
            </dl>
            ...
            <div className="mt-auto min-w-0 border-t border-[#ececf0] pt-4">
              <p className="text-[11px] text-[#8a91a0] [overflow-wrap:anywhere]">{applicationStatusLabels[application.status]} · Son güncelleme {dateFormatter.format(new Date(application.updatedAt))}</p>
              {project && <Link to={`/projects/${project.id}/apply`} className="mt-4 inline-flex min-h-11 w-full min-w-0 items-center justify-center gap-2 rounded-xl border border-[#d8d6ec] bg-[#faf9ff] px-4 text-center text-sm font-semibold text-[#5148c7] hover:bg-[#f1efff]"><span className="min-w-0 [overflow-wrap:anywhere]">{cta}</span><ArrowRight className="size-4 shrink-0" /></Link>}
            </div>
          </article>;
        })}
      </section>
