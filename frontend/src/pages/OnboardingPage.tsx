...
export default function OnboardingPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    saveProfile({ ...draft, name: draft.name.trim(), headline: draft.headline.trim(), bio: draft.bio.trim(), location: draft.location.trim(), completedAt: isEditMode && profile ? profile.completedAt : new Date().toISOString() });
    clearProfileDraft();
  ...
  if (isCompletedView) {
    return <main className="min-h-screen bg-[#f7f6f2] text-[#17233e]">
      ...
      <section className="mx-auto flex max-w-[620px] flex-col items-center px-5 py-20 text-center sm:py-28">
        ...
        <p className="mt-4 max-w-lg text-sm leading-6 text-[#687184]">Tamamlanmış profilin bu tarayıcıda güvenle saklanıyor. Devam edebilir veya bilgilerini bilinçli olarak düzenleme modunda açabilirsin.</p>
  ...
  return <main className="min-h-screen bg-[#f7f6f2] text-[#17233e]">
    ...
    <div className="mx-auto grid max-w-[1040px] gap-10 px-5 py-10 lg:grid-cols-[280px_1fr] lg:py-16">
      ...
      <form onSubmit={submit} className="rounded-3xl border border-[#e0e0e6] bg-white p-6 shadow-[0_18px_50px_rgba(37,44,72,0.06)] sm:p-9">
        ...
        <div className="mt-9 flex items-center justify-between border-t border-[#ececf0] pt-6">{step > 0 ? <button type="button" onClick={() => setStep((current) => current - 1)} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-semibold text-[#596477] hover:bg-[#f6f5f2]"><ArrowLeft className="size-4" />Geri</button> : <span />}<button type="submit" disabled={!canContinue} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#5448d8] px-5 text-sm font-semibold text-white shadow-[0_8px_18px_rgba(84,72,216,0.18)] transition hover:bg-[#463ac8] disabled:cursor-not-allowed disabled:opacity-45">{step === 2 ? isEditMode ? "Profili kaydet" : "Profili tamamla" : "Devam et"}<ArrowRight className="size-4" /></button></div>
      </form>
    </div>
  </main>;
}
