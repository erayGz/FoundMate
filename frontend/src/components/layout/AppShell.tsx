import { ClipboardList, Compass, FolderKanban, Home, LogOut, Menu, UsersRound, X } from "lucide-react";
  ...
  return <div className="min-h-screen bg-[#f7f6f2] text-[#17233e]">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[260px] border-r border-[#e4e3df] bg-[#fdfcf8] p-5 lg:flex lg:flex-col">
      ...
      <div className="mt-auto">
        {profile ? <Link to="/onboarding?mode=edit" className="rounded-2xl border border-[#e2e2e8] bg-white p-3.5 transition hover:border-[#c8c4ec]"><div className="flex items-center gap-3"><span className="inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-[#e6e2ff] text-xs font-bold text-[#4d43c2]">{initials}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-[#26334f]">{profile.name}</p><p className="truncate text-[11px] text-[#758094]">{profile.headline}</p></div></div></Link> : <Link to="/onboarding" className="flex rounded-2xl border border-[#d8d5ed] bg-[#f5f3ff] p-4 text-sm font-semibold text-[#5148c7]">Başvuru için profilini tamamla</Link>}
        <button type="button" onClick={handleLogout} className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl px-3.5 text-sm font-semibold text-[#7a8394] transition hover:bg-[#f6f5f1] hover:text-[#26334f]"><LogOut className="size-[18px]" />Çıkış yap</button>
      </div>
    </aside>
